...
describe FerriesScraper::Scraper do
  ...
  describe "Configuration for desired departure city" do
    it "allows Dublin" do
      expect { FerriesScraper::Scraper.new({ departure: 'Dublin' }, strategy) }.
        not_to raise_error
    end

    it "allows Liverpool" do
      expect { FerriesScraper::Scraper.new({ departure: 'Liverpool' }, strategy) }.
        not_to raise_error
    end

    it "won't allow other cities" do
      expect { FerriesScraper::Scraper.new({ departure: 'Holyhead' }, strategy) }.
  ...
  describe "Configuration for scraping strategy" do
    it "injects scraping strategy" do
      FerriesScraper::Scraper.new({ departure: 'Dublin' }, strategy).strategy.should be(strategy)
    end
  end

  describe "#scrape" do
    subject { FerriesScraper::Scraper.new({ departure: 'Dublin' }, strategy) }
    ...
    it "scrapes the correct url for the given departure city" do
      FerriesScraper::Scraper::ROUTE_RFIDS.each do |city, id|
        scraper = FerriesScraper::Scraper.new({ departure: "#{city}" }, strategy)
